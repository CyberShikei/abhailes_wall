...
class DataCleaner:
    ...
    def sanitize_column_names(self):
        ...
        cleaned_columns = {col: clean_column_name(
    ...
    def _get_sql_data_types(self):
        ...
        type_mapping = {
            ...
            # complex numbers can be stored as strings
            'complex128': 'VARCHAR(255)'
        }
        ...
        for column, dtype in self.dataframe.dtypes.items():
            ...
            if 'datetime' in dtype_str:
                ...
                if dtype_str == 'object':
                    avg_length = self.dataframe[column].apply(
                        lambda x: len(str(x)) if x is not None else 0).mean()

                    # Set the length for VARCHAR based on the average length, with a buffer
                    if avg_length > 255:
                        sql_type = 'TEXT'  # Use TEXT for large strings
                    else:
                        # Add buffer to average length
                        optimal_length = int(avg_length) + 50
    ...
    def createTblStr_sql(self, table_name: str) -> str:
        ...
        columns = [f"  {col_name} {data_type}" for col_name,
                   data_type in sql_types.items()]
